...
**DO NOT edit these files directly** - they are regenerated by `render_docs.py` and your changes will be lost:
- `docs/index.html` - Generated redirect
- `docs/releases/*.html` - Generated from `data/releases.json`
- `docs/features/*.html` - Generated from `data/features.json`
...
2. **Never edit generated HTML directly** - Edit `scripts/render_docs.py` instead (see "Generated vs Source Files" above)
3. **AI proposes, tools apply** - Generate mutation payloads for human confirmation, then execute via `mutate.py`
4. **Versioning** - Epics and stories are versioned; prior versions are immutable once superseded
5. **Traceability** - Downstream artifacts reference upstream by stable IDs
6. **Explicit timeline ownership** - Every version is bound to exactly one release
7. **CRITICAL: Regenerate HTML after data changes** - The `docs/` folder contains generated HTML that must be rebuilt after any changes to `data/*.json` or `scripts/`. See "Build Process" section below.
